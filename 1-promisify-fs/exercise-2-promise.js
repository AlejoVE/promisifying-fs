...
readFilePromise(sourcePath, "utf-8")
  .then((originalContent) => {
    const originalSourceContent = originalContent;
    log(4, `copying to ${fileName2} ...`);
    copyFilePromise(sourcePath, targetPath).then(() => {
      log(5, `reading ${fileName1} ...`);
      readFilePromise(sourcePath, "utf-8").then((content) => {
        const sourceContent = content;
        log(6, `asserting ${fileName1} ...`);
        assert.strictEqual(sourceContent, originalSourceContent);
        log(7, `reading ${fileName2} ...`);
        readFilePromise(targetPath, "utf-8").then((contentTarget) => {
          const targetContent = contentTarget;
...
// pass: 5/13/2020, 18:45:16

// pass: 5/13/2020, 18:53:25